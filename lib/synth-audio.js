 ...
 */
async function synthAudio(client, logger, stats, {vendor, language, voice, gender, text, salt, credentials}) {
  let audioBuffer;
  ...
  if (!result) {
    ...
    audioBuffer = 'google' === vendor ?
      await synthGoogle({credentials, stats, language, voice, gender, text}) :
      await synthPolly({credentials, stats, language, voice, text});
...
const synthPolly = ({credentials, stats, language, voice, text}) => {
...
const synthGoogle = async({credentials, stats, language, voice, gender, text}) => {

...
async function synthAudio(client, logger, stats, {
  vendor, language, voice, gender, text, engine, salt, model, credentials
}) {
  let audioBuffer;
  let servedFromCache = false;
  logger = logger || noopLogger;

  assert.ok(['google', 'aws', 'polly', 'microsoft', 'wellsaid', 'nuance'].includes(vendor),
  ...
  if ('google' === vendor) {
    ...
    assert.ok(voice, 'synthAudio requires voice when microsoft is used');
  }
  ...
  if (!result) {
    ...
    if ('microsoft' === vendor) return {filePath, servedFromCache};
  }

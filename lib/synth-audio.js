...
async function synthAudio(client, logger, stats, {vendor, language, voice, gender, text, salt, credentials}) {
  ...
  if (!result) {
    ...
    switch (vendor) {
      ...
      case 'aws':
      case 'polly':
        audioBuffer = await synthPolly(logger, {credentials, stats, language, voice, text});
        break;
      case 'microsoft':
...
const synthPolly = (logger, {credentials, stats, language, voice, text}) => {
...
const synthMicrosoft = async(logger, {credentials, stats, language, voice, text, filePath}) => {
  try {
    ...
    /*
    if (text.startsWith('<speak>') {...build SSML..}
    */

    return new Promise((resolve, reject) => {
      const speakAsync = text.startsWith('<speak') ?

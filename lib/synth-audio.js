...
const {makeSynthKey, createNuanceClient, noopLogger} = require('./utils');
...
async function synthAudio(client, logger, stats, {
  ...
  assert.ok(['google', 'aws', 'polly', 'microsoft', 'wellsaid', 'nuance', 'ibm'].includes(vendor),
    `synthAudio supported vendors are google, aws, microsoft, nuance, and wellsaid, not ${vendor}`);
  if ('google' === vendor) {
    ...
    assert.ok(credentials.region, 'synthAudio requires region in credentials when ibm watson is used');
    assert.ok(credentials.api_key, 'synthAudio requires api_key in credentials when nuance is used');
  }
...
const synthIbm = async(logger, {credentials, stats, voice, text}) => {
  ...
  try {
    const textToSpeech = new TextToSpeechV1({
      ...
      serviceUrl: `https://api.${region}.text-to-speech.watson.cloud.ibm.com`
    });

...
async function getNuanceAccessToken(client, logger, clientId, secret, scope) {
  ...
  try {
    ...
    await client.set(key, json.access_token, 'EX', json.expires_in);
    return {...json, servedFromCache: false};
  } catch (err) {
    debug(err, `listCallInfo: Error retrieving Nuance access token for client_id ${clientId}`);
    logger.error(err, `listCallInfo: Error retrieving Nuance access token for client_id ${clientId}`);
  }
}
